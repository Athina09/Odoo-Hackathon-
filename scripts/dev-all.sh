#!/usr/bin/env bash
# Start EcoSphere frontend, API, and RAG dashboard together.
set -euo pipefail
...
if port_busy 8000; then
  ...
  echo "Starting EcoSphere API (uvicorn :8000)..."
...
if port_busy 8501; then
  ...
  echo "Starting ESG RAG dashboard (streamlit :8501)..."
